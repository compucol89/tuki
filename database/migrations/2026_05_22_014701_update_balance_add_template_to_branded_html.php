...
{
    /**
     * Update balance_add template to branded HTML with corporate design.
     */
...
<head>
  ...
  <title>Saldo acreditado — {website_title}</title>
  <style>
    body {
...
<body>
  <div class="container">
    <div class="header">
      <h1>{website_title}</h1>
      <p>Saldo acreditado</p>
      <div class="badge">Dinero en tu cuenta</div>
    </div>
    <div class="body">
      <p>Hola <strong>{username}</strong>,</p>
      <p>
        Te informamos que se acreditó saldo en tu cuenta de <strong>{website_title}</strong>.
      </p>
      <div class="details-box">
        <p><strong>Monto acreditado:</strong> {amount}</p>
        <p><strong>Saldo actual:</strong> {current_balance}</p>
        <p><strong>ID de transacción:</strong> {transaction_id}</p>
      </div>
      <p style="margin-bottom: 0;">
        El saldo ya está disponible para usar en la plataforma.
      </p>
    ...
    public function down(): void
    {
        DB::table('mail_templates')
            ...
            ->where('mail_type', 'balance_add')
            ->update([
                'mail_subject' => 'Balance Add',
                'mail_body' => '<p>Hi {username}</p><p>{amount} added to your account.</p><p>Your current balance is {current_balance}. </p><p>The transaction id is {transaction_id}.<br /></p><p><br /></p><p>Best Regards.<br />{website_title}<br /></p>',
            ]);
    }
};
