...
class PaystackController extends Controller
{
  ...
  public function notify(Request $request)
  {
    ...
    if (empty($reference) || empty($arrData) || empty($eventId)) {
    ...
    // El status de la transacción debe ser 'success' según la API
    if (($verification['status'] ?? false) !== true || ($verification['data']['status'] ?? '') !== 'success') {
      $request->session()->forget(['eventId', 'arrData', 'discount']);
      return redirect()->route('event_booking.cancel', ['id' => $eventId]);
    }

    // Verificar monto: Paystack devuelve el monto en centavos (kobo/pesewas)
    $expectedAmount = round((float)$arrData['price'] + (float)$arrData['tax'], 2);
    ...
    $request->session()->forget(['event_id', 'eventId', 'selTickets', 'arrData', 'paymentId', 'discount']);
