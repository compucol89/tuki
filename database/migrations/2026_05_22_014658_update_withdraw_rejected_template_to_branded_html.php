...
{
    /**
     * Update withdraw_rejected template to branded HTML with corporate design.
     */
    public function up(): void
    {
        DB::table('mail_templates')
            ->where('id', 14)
            ->where('mail_type', 'withdraw_rejected')
            ->update([
                'mail_subject' => '{website_title} — Retiro rechazado',
                'mail_body' => '<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Retiro rechazado — {website_title}</title>
  <style>
    body {
...
<body>
  <div class="container">
    <div class="header">
      <h1>{website_title}</h1>
      <p>Retiro rechazado</p>
      <div class="badge">Solicitud cancelada</div>
    </div>
    <div class="body">
      <p>Hola <strong>{organizer_username}</strong>,</p>
      <p>
        Te informamos que tu solicitud de retiro <strong>#{withdraw_id}</strong> fue rechazada y el monto fue reintegrado a tu cuenta en <strong>{website_title}</strong>.
      </p>
      <div class="details-box">
        <p><strong>Saldo actual:</strong> {current_balance}</p>
      </div>
      <p style="margin-bottom: 0;">
        Si tenés alguna duda sobre el motivo del rechazo, contactanos desde tu panel de organizador.
      </p>
    ...
    public function down(): void
    {
        DB::table('mail_templates')
            ...
            ->where('mail_type', 'withdraw_rejected')
            ->update([
                'mail_subject' => 'Withdraw Request Rejected',
                'mail_body' => '<p>Hi {organizer_username},</p><p>This email confirms that your withdrawal request  {withdraw_id} is rejected and the balance added to your account. </p><p>Your current balance is {current_balance}</p><p><br /></p><p>Best Regards.<br />{website_title}</p>',
            ]);
    }
};
