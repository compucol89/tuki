          ...
          <div class="col-lg-4">
            ...
            <div class="checkout-product mb-25">
              ...
              <div class="content">
                ...
                <span> <i class="fas fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($event->start_date)->translatedFormat('D d/m/Y') }} &nbsp;
            ...
            <div>
              <div id="couponReload">
                ...
                <ul class="package-summary mb-25">
                  @if ($selTickets != null)
                    @php
                      $ticketIds = array_column($selTickets, 'ticket_id');
                      $ticketsMap = App\Models\Event\Ticket::whereIn('id', $ticketIds)
                          ->select('id', 'pricing_type')->get()->keyBy('id');
                      $ticketContentsMap = App\Models\Event\TicketContent::whereIn('ticket_id', $ticketIds)
                          ->where('language_id', $currentLanguageInfo->id)
                          ->select('ticket_id', 'title')->get()->keyBy('ticket_id');
                      $ticketContentsDefault = App\Models\Event\TicketContent::whereIn('ticket_id', $ticketIds)
                          ->select('ticket_id', 'title')->get()->keyBy('ticket_id');
                    @endphp
                    <li>
                    ...
                    @foreach ($selTickets as $selTicket)
                      @php
                        ...
                        if ($t && $t->pricing_type == 'variation') {
                            $varContent = App\Models\Event\VariationContent::where([
                                ['ticket_id', $selTicket['ticket_id']],
                                ['name', $selTicket['name']],
                                ['language_id', $currentLanguageInfo->id]
                            ])->value('name');
                            $name = $varContent ?: $selTicket['name'];
                        } else {
                            $tc = $ticketContentsMap[$selTicket['ticket_id']]
                                ?? $ticketContentsDefault[$selTicket['ticket_id']]
                                ?? null;
                            $name = $tc ? $tc->title : '';
                        }
                      @endphp
                  ...
                  @php
                    ...
                    Session::put('tax', $tax);
                    Session::put('grand_total', $sub_total + $tax - ($discount + $total_early_bird_dicount));
                  @endphp
                </ul>
              </div>
            </div>
                  ...
                  @if (!is_null($offlineGateway->instructions))
                    <div class="form-group mb-4">
                      ...
                      <div class="summernote-content">
                        {!! clean($offlineGateway->instructions) !!}
                      </div>
                    </div>
                  @endif
