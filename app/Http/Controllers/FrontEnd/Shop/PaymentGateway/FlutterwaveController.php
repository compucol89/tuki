...
class FlutterwaveController extends Controller
{
    ...
    public function notify(Request $request)
    {
        try {
            ...
            // txref/tx_ref viene en los parámetros GET del redirect de Flutterwave
            $ref = $request->get('tx_ref') ?? $request->get('txref') ?? Session::get('txref') ?? '';

            if (empty($ref) || empty($arrData)) {
                $request->session()->forget(['arrData', 'paymentId']);
