...
{
    /**
     * Update event_booking_approved template to branded HTML with corporate design.
     */
    public function up(): void
    {
        DB::table('mail_templates')
            ->where('id', 10)
            ->where('mail_type', 'event_booking_approved')
            ->update([
                'mail_subject' => '{website_title} — Tu reserva fue aprobada',
                'mail_body' => '<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Tu reserva fue aprobada — {website_title}</title>
  <style>
    body {
...
<body>
  <div class="container">
    <div class="header">
      <h1>{website_title}</h1>
      <p>Tu reserva fue aprobada</p>
      <div class="badge">Pago confirmado</div>
    </div>
    <div class="body">
      <p>Hola <strong>{customer_name}</strong>,</p>
      <p>
        Te confirmamos que tu pago se completó correctamente y tu reserva fue aprobada en <strong>{website_title}</strong>.
      </p>
      ...
      <p style="margin-bottom: 0;">
        ¡Gracias por elegirnos! Te esperamos en el evento.
      </p>
    ...
    public function down(): void
    {
        DB::table('mail_templates')
            ...
            ->where('mail_type', 'event_booking_approved')
            ->update([
                'mail_subject' => 'Approval of Event Booking',
                'mail_body' => '<p>Hi <span style="font-weight:600;">{customer_name}</span>,</p><p>Your payment is completed, and we have approved your booking for the following evnent.</p><p>Booking Id: #{order_id}<br /></p><p>Event : {title}</p><p>Also, we have attached an invoice in this mail.</p><p>Best regards.<br />{website_title}</p>',
            ]);
    }
};
