  ...
  <div class="organizer-booking-admin">
    ...
    <section class="ob-type-summary" aria-labelledby="organizerTicketTypeSummaryTitle">
      <div class="ob-type-summary__head">
        <div>
          <h2 id="organizerTicketTypeSummaryTitle" class="ob-type-summary__title">{{ __('Ventas por tipo de entrada') }}</h2>
          <div class="ob-muted">{{ __('Agrupado por evento y tipo; respeta los filtros aplicados.') }}</div>
        </div>
        <div class="ob-muted">{{ __('Vendido') }} = {{ __('completado') }} + {{ __('gratis') }}</div>
      </div>
      <div class="ob-type-summary__body">
        @if (empty($ticketSalesSummary))
        ...
        @else
          <table class="table ob-type-table">
            ...
            <tbody>
              @foreach ($ticketSalesSummary as $summaryRow)
                <tr>
                  <td data-label="{{ __('Tipo') }}">
                    <span class="ob-type-name">{{ $summaryRow['ticket_name'] }}</span>
                    <span class="ob-type-event">{{ $summaryRow['event_title'] }}</span>
                  </td>
